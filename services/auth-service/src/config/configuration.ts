...
export interface AppConfig {
  ...
  jwt: {
    accessSecret: string;
    issuer: string;
    /** Refresh tokens are signed with their own secret so an access token can
     * never be replayed on /auth/refresh (falls back to the access secret). */
...
export default (): AppConfig => ({
  ...
  jwt: {
    ...
    refreshTtlSec: parseInt(
      process.env.JWT_REFRESH_TTL_SEC || String(30 * 24 * 3600),
      10,
    ),
  },
});
